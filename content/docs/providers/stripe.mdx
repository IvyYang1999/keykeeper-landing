...
|---|---|
| Field | `stripe-api-key` → `STRIPE_API_KEY` in `keykeeper run` |
| Key looks like | starts with `rk_live_` or `rk_test_` or `sk_live_` or `sk_test_`, at least 32 characters, shown once when created |
...
`--provider stripe` fills in the credential id and the field. The field's environment
...
- **Before writing:** the value must look like a Stripe key (starts with `rk_live_` or `rk_test_` or `sk_live_` or `sk_test_`, at least 32 characters). A wrong paste is refused before anything is stored, and the agent is told why — without the value.
...
No expiry; Rotate key gives the old value up to 7 days of grace. A key unused for 180 days gets restricted. Rotate or revoke at [dashboard.stripe.com](https://dashboard.stripe.com/apikeys).
