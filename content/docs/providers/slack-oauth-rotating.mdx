...
- **Before writing:** the value must look like a Slack · OAuth rotating token bundle key (starts with `xoxe.xoxb-`, whether it can be viewed again is unconfirmed). A wrong paste is refused before anything is stored, and the agent is told why — without the value.
